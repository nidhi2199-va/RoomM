...
public class BookingsDaoImpl implements BookingsDao {
    ...
    public List<Bookings> findByRoomAndStatus(MeetingRooms room, BookingStatus status) {
        ...
        query.setParameter("status", status); // Use a single status parameter
...
//    @Override
//    public List<Bookings> findByUserIdAndTimeslot(Long userId, long epochTime) {
//        LocalDateTime dateTime = Instant.ofEpochMilli(epochTime).atZone(ZoneId.systemDefault()).toLocalDateTime();
//
//        String hql = "FROM Bookings b WHERE b.user.id = :userId AND b.endTime < :time AND b.status = :status";
//
//        return entityManager.createQuery(hql, Bookings.class)
//                .setParameter("userId", userId)
